...
final class HFNoiseReductionProcessor: NoiseReductionProcessor {
    ...
    func processFrame48kMonoInPlace(_ frame: inout [Float]) {
        ...
        while inQ.count >= K.hopN {
            var hop = Array(inQ.prefix(K.hopN))
            inQ.removeFirst(K.hopN)
            if config.impulseBlanker { blankerProcess(&hop) }
            outQ.append(contentsOf: olaProcess(hop))
        }
        guard outQ.count >= frame.count else { return }
        frame = Array(outQ.prefix(frame.count))
        outQ.removeFirst(frame.count)
    }
    ...
    private func olaProcess(_ hop: [Float]) -> [Float] {
        ...
        // 6. Wiener gain with decision-directed SNR.
        let gain = computeGain()

        // 7. Apply gain and passband mask.
        for k in 0..<bins {
            let g = (k < pbBins) ? gain[k] : Float(0)
            realBuf[k] *= g
            imagBuf[k] *= g
        }
        ...
        // 12. Output first h samples; shift accumulator.
        let outHop = Array(olaAcc.prefix(h))
    ...
    private func computeGain() -> [Float] {
        let bins  = K.binN
        let alpha = K.ddAlpha
        let floor = gainFloor
