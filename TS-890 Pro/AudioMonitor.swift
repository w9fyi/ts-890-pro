...
final class AudioMonitor {
    ...
    func start(inputDeviceID: AudioDeviceID, outputDeviceID: AudioDeviceID) throws {
        ...
        if let inputSR, abs(inputSR - sampleRate) > 1,
           !AudioDeviceManager.ensureNominalSampleRate(sampleRate, deviceID: inputDeviceID) {
            throw MonitorError.unsupportedSampleRate(inputSR)
        }

        let outputSR = AudioDeviceManager.nominalSampleRate(of: outputDeviceID)
        if let outputSR, abs(outputSR - sampleRate) > 1,
           !AudioDeviceManager.ensureNominalSampleRate(sampleRate, deviceID: outputDeviceID) {
    ...
    private func drainAndProcess() {
        ...
        while rawFifo.availableToRead() >= frameSize {
            ...
            // Copy dry → wet into wetFrame's existing (uniquely-owned) storage.
            // `wetFrame = dryFrame` would share dryFrame's buffer, forcing a
            // copy-on-write reallocation when the processor mutates it in place.
            wetFrame.withUnsafeMutableBufferPointer { dst in
                dryFrame.withUnsafeBufferPointer { src in
                    dst.baseAddress!.update(from: src.baseAddress!, count: frameSize)
                }
            }
