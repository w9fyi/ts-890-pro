...
struct FT8SectionView: View {
    ...
    private func startFT8() {
        ...
        if forceUSB {
            radio.setOperatingMode(.usb)  // model-aware: OM on 890/990, MD on 590
        }
        if forceDataMode {
            // TS-890 appears to use MD for USB-DATA (DA is rejected by the radio).
            radio.send(KenwoodCAT.setModeMD(2))
            if radio.capabilities.hasDataModeCommand {
                // TS-590S/SG: data mode is the separate DA overlay.
                radio.send(KenwoodCAT.setDataMode(enabled: true))
            }
            radio.send(KenwoodCAT.getModeMD())
        }
        // Route TX audio through LAN VoIP so FT8 tones reach the modulator.
        // (MS is a TS-890S/990S command; the 590 has no LAN audio path.)
        if radio.capabilities.hasAudioSourceSelect {
            radio.send("MS003;")
        }
    ...
    private func stopFT8() {
        ...
        if let mode = vm.preFT8Mode {
            ...
            AppFileLogger.shared.log("FT8: restore mode \(mode.label)")
            radio.setOperatingMode(mode)  // model-aware: OM on 890/990, MD+DA on 590
        }
        if let md = vm.preFT8MDMode, !radio.capabilities.is590Family {
