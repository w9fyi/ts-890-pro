...
final class SerialCATConnection: CATTransport {
    ...
    // TS-590S/SG (and TS-990S) require a leading "0" parameter on AG/SQ reads;
    // bare "AG;"/"SQ;" are errors there. Only accessed on `queue`.
    private var zeroPrefixQueries = false

    /// Switch the keepalive AG/SQ polls to the zero-prefixed form. Safe to call from any thread.
    func updateZeroPrefixQueries(_ enabled: Bool) {
        queue.async { [weak self] in self?.zeroPrefixQueries = enabled }
    }
    ...
    private func startKeepalive() {
        ...
        t.setEventHandler { [weak self] in
            ...
            switch tickCount % 2 {
            case 0:
                self.sendRaw(self.zeroPrefixQueries ? KenwoodCAT.getAFGain(band: 0)
                                                    : KenwoodCAT.getAFGain())
                self.sendRaw(KenwoodCAT.getRFGain())
            default:
                self.sendRaw(self.zeroPrefixQueries ? KenwoodCAT.getSquelchLevel(band: 0)
                                                    : KenwoodCAT.getSquelchLevel())
            }
