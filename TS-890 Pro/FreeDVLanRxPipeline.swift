...
final class FreeDVLanRxPipeline {
    ...
    func feed16kSamples(_ samples: [Int16]) {
        // Decimate 16→8 kHz directly into the reused buffer (keep every other
        // sample) — no intermediate `decimated` array.
        modem8kBuffer.removeAll(keepingCapacity: true)
        for (i, s) in samples.enumerated() {
            if (i + decimCounter) % 2 == 0 { modem8kBuffer.append(s) }
        }
    ...
    // Precomputed interpolation weights k/n for k in 0..<upsampleFactor, so the
    // hot path avoids a divide per output sample.
    private lazy var upWeights: [Float] = (0..<upsampleFactor).map { Float($0) / Float(upsampleFactor) }
    ...
    private func appendUpsampled(from a: Float, to b: Float, into out: inout [Float]) {
        let d = b - a
        for w in upWeights { out.append(a + d * w) }
    }
}
