...
enum AudioDeviceManager {
    ...
    static func nominalSampleRate(of deviceID: AudioDeviceID) -> Double? {
        getScalarProperty(deviceID, selector: kAudioDevicePropertyNominalSampleRate)
    }

    /// Switches the device to the given nominal sample rate if it isn't there
    /// already. Radio USB codecs (e.g. the TS-590S's PCM2901) often enumerate at
    /// 44.1 kHz on macOS while every audio pipeline in this app requires 48 kHz.
    /// The HAL applies the change asynchronously, so poll briefly for it to take
    /// effect. Returns true when the device ends up at the requested rate.
    @discardableResult
    static func ensureNominalSampleRate(_ rate: Double, deviceID: AudioDeviceID, timeout: TimeInterval = 1.5) -> Bool {
        if let current = nominalSampleRate(of: deviceID), abs(current - rate) < 1 { return true }
        var address = AudioObjectPropertyAddress(
