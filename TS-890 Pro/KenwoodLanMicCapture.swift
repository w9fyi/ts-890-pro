...
final class KenwoodLanMicCapture {
    ...
    private var pending16kReadOffset = 0
    private var frameScratch = [Int16](repeating: 0, count: 320)
    ...
    private func handleInput(ioActionFlags: UnsafeMutablePointer<AudioUnitRenderActionFlags>, inTimeStamp: UnsafePointer<AudioTimeStamp>, inBusNumber: UInt32, inNumberFrames: UInt32) -> OSStatus {
        ...
        return scratch.withUnsafeMutableBufferPointer { bufPtr in
            ...
            let buffer = AudioBuffer(mNumberChannels: channels, mDataByteSize: byteCount, mData: base)
    ...
    private func consume48kMono(_ ptr: UnsafePointer<Float>, frames: Int) {
        ...
        while pending16k.count - pending16kReadOffset >= 320 {
            frameScratch.withUnsafeMutableBufferPointer { dst in
                pending16k.withUnsafeBufferPointer { src in
                    dst.baseAddress!.update(from: src.baseAddress!.advanced(by: pending16kReadOffset), count: 320)
                }
            }
            pending16kReadOffset += 320
        ...
        compactPending16kIfNeeded()
    }

    private func compactPending16kIfNeeded() {
        guard pending16kReadOffset > 0 else { return }
        if pending16kReadOffset == pending16k.count {
            pending16k.removeAll(keepingCapacity: true)
            pending16kReadOffset = 0
        } else if pending16kReadOffset >= 320 * 8 {
            pending16k.removeFirst(pending16kReadOffset)
            pending16kReadOffset = 0
        }
    }
}
