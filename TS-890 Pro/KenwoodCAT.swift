...
enum KenwoodCAT {
    ...
    // MARK: - TS-590S/SG dialect variants
    //
    // The TS-590S/SG use an older CAT dialect. Several of the TS-890S read
    // queries above are SET commands on the 590 (NB1; PR0; SC1; FL00;), so the
    // 590 variants below must be used when KenwoodCapabilities.is590Family.
    // Source: TS-590S/SG PC Control Command Reference rev3 (Jan 2019).

    /// IF; — full transceiver status (freq, RIT/XIT, TX state, mode, split, scan).
    /// The only way to read the RIT/XIT offset on the 590. Absent on the TS-890S.
    static func getTransceiverStatus() -> String { "IF;" }

    /// RA set on the 590: RA{2 digits} — 00=ATT off, 01=ATT on (single 12 dB step).
    static func setAttenuator590(on: Bool) -> String { on ? "RA01;" : "RA00;" }

    /// GC on the 590: 0=OFF, 1=SLOW, 2=FAST (there is no MID).
    static func setAGC590(_ mode: AGCMode) -> String {
        switch mode {
        case .off:  "GC0;"
        case .slow: "GC1;"
        default:    "GC2;"  // .mid and .fast both map to FAST
        }
    }

    /// SL/SH on the 590: bare 2-digit codes 00–13, tables depend on mode.
    static func getReceiveFilterLowCut590() -> String { "SL;" }
    static func setReceiveFilterLowCut590(_ id: Int) -> String {
        String(format: "SL%02d;", max(0, min(id, 13)))
    }
    static func getReceiveFilterHighCut590() -> String { "SH;" }
    static func setReceiveFilterHighCut590(_ id: Int) -> String {
        String(format: "SH%02d;", max(0, min(id, 13)))
    }

    /// FW on the 590: DSP filter width in Hz (CW/FSK only; FM narrow flag).
    /// Do NOT send to a TS-890S — FW means FM Normal/Narrow there.
    static func getFilterWidth590() -> String { "FW;" }
    static func setFilterWidth590(hz: Int) -> String {
        String(format: "FW%04d;", max(0, min(hz, 9999)))
    }

    /// IS set on the 590: P1 is always a space, CW/CW-R only (300–1000 Hz).
    static func setReceiveFilterShiftHz590(_ hz: Int) -> String {
        String(format: "IS %04d;", max(0, min(hz, 9999)))
    }

    /// FL on the 590: read "FL;", set FL{n} where 1=IF Filter A, 2=IF Filter B.
    static func getFilterSlot590() -> String { "FL;" }
    static func setFilterSlot590(_ slot: FilterSlot) -> String {
        slot == .b ? "FL2;" : "FL1;"
    }

    /// NB on the 590: 0=off, 1=NB1, 2=NB2, 3=both. Read is bare "NB;".
    static func getNoiseBlanker590() -> String { "NB;" }
    static func setNoiseBlanker590(_ value: Int) -> String {
        "NB\(max(0, min(value, 3)));"
    }

    /// NL on the 590: level 001–010 for whichever NB is active (3 digits).
    static func getNoiseBlankerLevel590() -> String { "NL;" }
    static func setNoiseBlankerLevel590(_ level: Int) -> String {
        String(format: "NL%03d;", max(1, min(level, 10)))
    }

    /// RL on the 590: NR level, 2 digits (NR1: 01–10, NR2: 00–09). Read is bare "RL;".
    static func getNRLevel590() -> String { "RL;" }
    static func setNRLevel590(_ level: Int) -> String {
        String(format: "RL%02d;", max(0, min(level, 10)))
    }

    /// PR on the 590: bare read/set (PR0; would SET the processor off).
    static func getSpeechProc590() -> String { "PR;" }
    static func setSpeechProc590(enabled: Bool) -> String { "PR\(enabled ? 1 : 0);" }

    /// SC on the 590: bare read. Never send SC1; — that starts a scan.
    static func getScanState590() -> String { "SC;" }
    static func setScanEnabled590(_ on: Bool) -> String { "SC\(on ? 1 : 0);" }

    /// NT on the 590 takes a bandwidth P2: NT{P1}{P2}. P1: 0=off, 1=auto, 2=manual.
    static func setNotch590(enabled: Bool) -> String { "NT\(enabled ? 1 : 0)0;" }

    /// LK set on the 590 takes two digits (P2 always 0).
    static func setLock590(_ on: Bool) -> String { "LK\(on ? 1 : 0)0;" }

    /// AN set on the 590 takes three parameters (port, rxAnt, driveOut; 9 = no change).
    static func setAntenna590(port: Int, rxAnt: Int, driveOut: Int) -> String {
        "AN\(port)\(rxAnt)\(driveOut);"
    }

    /// BD on the 590: set-only 2-digit band select (00=1.8 MHz … 09=50 MHz, 10=GENE).
    static func setBand590(_ band: Int) -> String {
        String(format: "BD%02d;", max(0, min(band, 10)))
    }

    /// VD on the 590: VOX delay in milliseconds, 4 digits 0000–3000 (steps of 150).
    static func getVOXDelay590() -> String { "VD;" }
    static func setVOXDelay590(ms: Int) -> String {
        String(format: "VD%04d;", max(0, min(ms, 3000)))
    }

    /// VG on the 590: VOX gain 000–009, 3 digits. No anti-VOX command.
    static func getVOXGain590() -> String { "VG;" }
    static func setVOXGain590(_ gain: Int) -> String {
        String(format: "VG%03d;", max(0, min(gain, 9)))
    }

    // MARK: TS-590 memory channels (MC / MR / MW; memory mode via FR2)

    /// MC; — read current memory channel number (answer MC{P1}{P2P2}, P1 may be a space).
    static func getMemoryChannelNumber590() -> String { "MC;" }
    static func setMemoryChannelNumber590(_ channel: Int) -> String {
        String(format: "MC%03d;", max(0, min(channel, 119)))
    }

    /// MR{split}{ch}; — read one memory channel record (50-char answer).
    /// split: false = simplex/RX data, true = split TX data.
    static func getMemoryChannel590(_ channel: Int, split: Bool = false) -> String {
        String(format: "MR%d%03d;", split ? 1 : 0, max(0, min(channel, 119)))
    }

    /// Memory mode on the 590 is entered with FR2 (memory) and left with FR0 (VFO A).
    static func setMemoryMode590(_ enabled: Bool) -> String { enabled ? "FR2;" : "FR0;" }

    /// MW — write a memory channel (fixed-layout record, simplex form).
    /// mode is the MD digit (1–9); data-mode is the separate P6 flag.
    /// Layout after "MW": split(1) ch(3) freq(11) mode(1) data(1) tone(1)
    /// tone#(2) ctcss#(2) "000" filter(1) "0" zeros(9) narrow(2) lockout(1) name(≤8).
    static func writeMemoryChannel590(channel: Int, hz: Int, mode: Int,
                                      dataMode: Bool = false, fmNarrow: Bool = false,
                                      lockout: Bool = false, name: String = "") -> String {
        let ch = max(0, min(channel, 119))
        let freq = max(0, min(hz, 99_999_999_999))
        let safeName = name.replacingOccurrences(of: ";", with: " ")
        let trimmed = String(safeName.prefix(8))
        // P7–P13 are all zero for a plain simplex write: tone off, tone/CTCSS
        // number 00, "000", filter A, "0", nine reserved zeros = 19 zeros.
        let zeros = String(repeating: "0", count: 19)
        return String(format: "MW0%03d%011d%d%d%@%02d%d%@;",
                      ch, freq, mode, dataMode ? 1 : 0, zeros,
                      fmNarrow ? 1 : 0, lockout ? 1 : 0, trimmed)
    }

    // MARK: TS-590 digital-mode helpers

    /// Enter USB-DATA on the 590: MD2 (USB) + DA1 (data mode on).
    /// The rear input (ACC2 vs USB codec) is selected by the "DATA modulation
    /// line" menu — see KenwoodCapabilities.dataModulationLineMenu.
    static func configureForDigitalMode590() -> [String] {
        ["MD2;", "DA1;"]
    }

    /// TX1 = DATA SEND: transmits using the ACC2/USB input instead of the mic.
    /// Same semantics on the TS-890S, but required on the 590 (no MS routing).
    static func pttDownDataSend() -> String { "TX1;" }
}
