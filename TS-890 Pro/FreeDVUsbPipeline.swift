...
final class FreeDVUsbPipeline {
    ...
    /// Reused 10 ms (480-sample) chunk scratch — processRx() and processTx() run
    /// serially on processQueue, so a single buffer is safe to share.
    private var chunkScratch = [Float](repeating: 0, count: 480)

    /// Precomputed linear-interp weights k/n, so the upsample hot loops avoid a
    /// divide per output sample. Modem is always 8 kHz → ×6 to 48 kHz; decoded
    /// speech is 8 or 16 kHz → ×rxUpsampleFactor.
    private lazy var rxUpWeights:    [Float] = (0..<rxUpsampleFactor).map { Float($0) / Float(rxUpsampleFactor) }
    private let modemUpWeights: [Float] = (0..<6).map { Float($0) / 6.0 }
    ...
    private func processRx() {
        ...
        while rxRaw.availableToRead() >= chunkSize {
            let got = chunkScratch.withUnsafeMutableBufferPointer { rxRaw.read(into: $0.baseAddress!, count: chunkSize) }
            guard got == chunkSize else { break }

            // Decimate 48→8 kHz (factor 6): take every 6th sample, straight into
            // the speech buffer (no intermediate array).
            var i = 0
            while i < chunkSize {
                rxSpeech8kBuf.append(Int16(clamping: Int32(chunkScratch[i] * 32767.0)))
                i += 6
            }
        }
        ...
        if !rxSpeech8kBuf.isEmpty {
            ...
            if !speech.isEmpty {
                ...
                var out = [Float](); out.reserveCapacity(speech.count * f)
                for i in 0 ..< max(0, speech.count - 1) {
                    let a = Float(speech[i]) / 32768.0
                    let d = Float(speech[i + 1]) / 32768.0 - a
                    for k in 0..<f { out.append(a + d * rxUpWeights[k]) }
                }
                rxLastSpeech = speech.last.map { Float($0) / 32768.0 }
    ...
    private func processTx() {
        ...
        while txRaw.availableToRead() >= chunkSize {
            let got = chunkScratch.withUnsafeMutableBufferPointer { txRaw.read(into: $0.baseAddress!, count: chunkSize) }
            guard got == chunkSize else { break }

            // Decimate 48 kHz → encoder speech rate (factor 6 for codec2, 3 for RADE).
            var i = 0
            while i < chunkSize {
                txSpeech8kBuf.append(Int16(clamping: Int32(chunkScratch[i] * 32767.0)))
                i += txDecimFactor
            }
        }
    ...
    private func enqueueModem8k(_ modem8k: [Int16]) {
        ...
        var out = [Float](); out.reserveCapacity(modem8k.count * 6)
        if let prev = txLastModem {
            let d = Float(modem8k[0]) / 32768.0 - prev
            for k in 0..<6 { out.append(prev + d * modemUpWeights[k]) }
        }
        for i in 0 ..< max(0, modem8k.count - 1) {
            let a = Float(modem8k[i]) / 32768.0
            let d = Float(modem8k[i + 1]) / 32768.0 - a
            for k in 0..<6 { out.append(a + d * modemUpWeights[k]) }
        }
        txLastModem = modem8k.last.map { Float($0) / 32768.0 }
