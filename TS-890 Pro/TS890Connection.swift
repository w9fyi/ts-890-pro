...
final class TS890Connection {
    private enum AuthState {
        case idle, sentCN, sentID, authenticated
    ...
    /// Cached trim set — building `.whitespacesAndNewlines.union(.controlCharacters)`
    /// allocates a CharacterSet, and frame parsing did it on every received frame.
    private static let frameTrimSet: CharacterSet = .whitespacesAndNewlines.union(.controlCharacters)

    /// Map an ASCII hex digit byte to its 0–15 value, or nil if not a hex digit.
    ...
    private func sanitizeCredential(_ s: String) -> String {
        s.trimmingCharacters(in: TS890Connection.frameTrimSet)
    }
    ...
    private func flushFrames() {
        ...
        while let separatorRange = receiveBuffer.firstRange(of: delimiter) {
            ...
            let cleaned = frame.trimmingCharacters(in: TS890Connection.frameTrimSet)
            ...
            if fullFrame.hasPrefix("##DD2") {
                ...
                if payload.count == 1280 {
                    // Decode 640 two-char hex bytes straight from the UTF-8 view —
                    // avoids 640 Substring allocations + radix:16 parses per frame.
                    var points = [UInt8]()
                    points.reserveCapacity(640)
                    var it = payload.utf8.makeIterator()
                    while let hi = it.next(), let lo = it.next() {
                        guard let h = TS890Connection.hexNibble(hi),
                              let l = TS890Connection.hexNibble(lo) else { continue }
                        points.append((h << 4) | l)
                    }
    ...
    private func handleAuthFrame(_ frame: String) {
        let normalized = frame.trimmingCharacters(in: TS890Connection.frameTrimSet)
        switch authState {
        case .sentCN:
