...
final class RADEEngine {
    ...
    private var rxScratch: [RADE_COMP] = []   // reused rade_rx input (sized ninMax)
    ...
    private var txFrameScratch: [Int16] = [] // reused 160-sample feature frame
    private var txFeatScratch: [Float] = []  // reused rade_tx() feature batch
    ...
    // Circular window of the most recent `hilbertNTaps` input samples. `histNewest`
    // indexes the most-recently-written sample; older samples sit at decreasing
    // indices (wrapping mod hilbertNTaps). A ring index avoids the O(n) array shift
    // that `removeFirst()` + `append()` incurred on *every* input sample.
    private var hist = [Float](repeating: 0, count: 127)
    private var histNewest = 0
    ...
    private func hilbertStep(_ x: Float) -> RADE_COMP {
        let n = RADEEngine.hilbertNTaps
        // advance ring head, store newest sample
        var newest = histNewest + 1
        if newest >= n { newest = 0 }
        histNewest = newest
        hist[newest] = x
        // hist[(newest - k) mod n] == x[n-k], matching coeffs[k]
        var imag: Float = 0
        hist.withUnsafeBufferPointer { h in
            RADEEngine.hilbertCoeffs.withUnsafeBufferPointer { c in
                var idx = newest
                for k in 0..<n {
                    imag += c[k] * h[idx]
                    idx -= 1
        ...
        var realIdx = newest - RADEEngine.hilbertDelay
        if realIdx < 0 { realIdx += n }
        let real = hist[realIdx]
    ...
    func open(mode: Mode = .radeV1) -> Bool {
        ...
        rxScratch = [RADE_COMP](repeating: RADE_COMP(real: 0, imag: 0), count: max(ninMax, 1))
        resetHilbert()
        ...
        txFrameScratch = [Int16](repeating: 0, count: max(Int(RADE_ENC_SAMPLES_PER_FRAME), 1))
        txFeatScratch  = [Float](repeating: 0, count: max(nFeatures, 1))
    ...
    func feedModemSamples(_ samples: [Int16]) -> [Int16] {
        ...
        while true {
            ...
            // Copy `nin` samples into the reused scratch (sized ninMax) rather
            // than allocating a fresh array each decode iteration.
            for i in 0..<nin { rxScratch[i] = complexBuffer[i] }
            complexBuffer.removeFirst(nin)

            var hasEoo: Int32 = 0
            let nOut = rxScratch.withUnsafeMutableBufferPointer { rxPtr in
    ...
    func encodeFromSpeech(_ speech: [Int16]) -> [Int16] {
        ...
        while txSpeechBuf.count >= frameLen {
            for i in 0..<frameLen { txFrameScratch[i] = txSpeechBuf[i] }
            txSpeechBuf.removeFirst(frameLen)
            txFrameScratch.withUnsafeBufferPointer { sp in
        ...
        while txFeatureBuf.count >= nFeatures {
            for i in 0..<nFeatures { txFeatScratch[i] = txFeatureBuf[i] }
            txFeatureBuf.removeFirst(nFeatures)
            let n = txFeatScratch.withUnsafeMutableBufferPointer { fp in
