...
struct ScopeView: View {
    ...
    private func drawBackground(ctx: GraphicsContext, size: CGSize) {
        ...
        // Accumulate every grid line into one Path and stroke once per frame.
        var grid = Path()
    ...
    private func drawSpectrum(ctx: GraphicsContext, size: CGSize, points: [UInt8]) {
        ...
        // Build the spectrum outline once, then derive the fill from it — avoids
        // iterating all points twice with identical x/y math every frame.
        var outline = Path()
        for (i, val) in points.enumerated() {
            let x = Double(i) / Double(count - 1) * w
            let y = Double(val) / Double(0x8C) * h
        ...
        fill.closeSubpath()
        ctx.fill(fill, with: .color(KenwoodTheme.spectrumFill))
